...
namespace lib3dfin
{
	class SectionExtractor
	{
	  ...
	  private: // methods
		...
		uint32_t innerCircle(const PointCloud2& circle_cloud, const Circle& circle_params) const;
		uint32_t sectorOccupancy(const PointCloud2& circle_cloud, const Circle& circle_params) const;
		// tilt dection for all sections of a given stem
		void tiltDetection(CircleSections& circles,
		                   const double    abs_weight_factor = 3.0,
		                   const double    rel_weight_factor = 1.0) const;
