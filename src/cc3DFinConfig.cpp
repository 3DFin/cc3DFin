...
namespace tdf
{
	...
	const std::vector<Field> Field::getConfigFields()
	{
		const size_t       numBasic    = basicFields.size();
		const size_t       numAdvanced = advancedFields.size();
		const size_t       numExpert   = expertFields.size();
		const size_t       numMisc     = miscFields.size();
		std::vector<Field> config_fields;
		config_fields.reserve(numBasic + numAdvanced + numExpert + numMisc);
		config_fields.insert(std::end(config_fields), std::begin(basicFields), std::end(basicFields));
		config_fields.insert(std::end(config_fields), std::begin(advancedFields), std::end(advancedFields));
		config_fields.insert(std::end(config_fields), std::begin(expertFields), std::end(expertFields));
		config_fields.insert(std::end(config_fields), std::begin(miscFields), std::end(miscFields));
